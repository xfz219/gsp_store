...
public class PushUtil extends Thread {
	...
	private static String ANDROID_MASTER_SECRET_USER = PropertiesLoader
			.getProperty("push.androiduseruser.mastersecret");
	private static String ANDROID_APP_KEY_CUSTOMER = PropertiesLoader.getProperty("push.androidcustomer.appkey");
	private static String ANDROID_MASTER_SECRET_CUSTOMER = PropertiesLoader
			.getProperty("push.androidcustomer.mastersecret");
	// ios
	private static boolean IOS_CERTIFICATE_ENVIRONMENT = Boolean
			.parseBoolean(PropertiesLoader.getProperty("push.ios.certificate.environment"));
	private static String IOS_CERTIFICATE_PATH_USER = PropertiesLoader.getProperty("push.iosuser.certificate.path");
	private static String IOS_CERTIFICATE_PATH_CUSTOMER = PropertiesLoader
			.getProperty("push.ioscustomer.certificate.path");
	...
	public static void sendMsg2IOS(List<String> tokens, String alert, int badge, Map<String, String> dictionarys,
		...
		try {
			...
			for (int i = 0; i < tokens.size(); i++) {
				devices.add(new BasicDevice(tokens.get(i)));
				if (devices.size() > 988 || i == tokens.size() - 1) {
					// 封装 payLoad 对象
					...
					if (pushtype.equals("IOStoCustomer")) {// 推送给个人
						pushManager.initializeConnection(
								new AppleNotificationServerBasicImpl(file + IOS_CERTIFICATE_PATH_CUSTOMER,
										IOS_CERTIFICATE_PASSWORD, IOS_CERTIFICATE_ENVIRONMENT));
					} else if (pushtype.equals("IOStoUser")) {// 推送给销售
						pushManager.initializeConnection(
								new AppleNotificationServerBasicImpl(file + IOS_CERTIFICATE_PATH_USER,
										IOS_CERTIFICATE_PASSWORD, IOS_CERTIFICATE_ENVIRONMENT));
					}
					// 3.推送消息
					...
					// 打印推送状态
					logger.info("推送[ios]消息.要推送总数={},成功={},失败={}", devices.size(), success.size(), failed.size());
					devices.clear();
				}
			}
		} catch (Exception e) {
	 ...
	 *            推送给设备的额外业务信息
	 */
	public static void sendMsg2Android(List<String> aliases, String alert, Map<String, String> extras,
			String pushtype) {
		...
		if (aliases != null && !aliases.isEmpty()) {
			...
			for (int i = 0; i < aliases.size(); i++) {
				aliase.add(aliases.get(i));
				if (aliase.size() > 988 || i == aliases.size() - 1) {
					logger.info("推送[android]消息到[{}]用户", aliase.size());
					b.setAudience(Audience.alias(aliase));
					// 设置发送自定义消息
