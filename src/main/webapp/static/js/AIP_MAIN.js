...
function AddSeal(usertype,doaction,other,httpaddr) {
	...
	if(usertype==0){
		...
		var islogin=AipObj.Login(other,3,65535,"","http://"+httpaddr+":8088/Seal/general/interface/");
	}else if(usertype==3){
	...
	AipObj.ForceSignType= 0x400;//盖上印章就雾化
...
function SaveTo(savetype,filepath,filecode) {
	...
	if(savetype==0){
		...
		AipObj.HttpAddPostString("id", filecode); //设置上传变量FileCode文档惟一标示。
